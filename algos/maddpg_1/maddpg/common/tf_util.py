...
def minimize_and_clip(optimizer, objective, var_list, clip_val=10):
    ...
    with tf.GradientTape() as tape:
        gradients = tape.gradient(objective, var_list)
        if clip_val is not None:
            clipped_gradients = [tf.clip_by_norm(grad, clip_val) if grad is not None else None for grad in gradients]
        else:
            clipped_gradients = gradients
    optimizer.apply_gradients(zip(clipped_gradients, var_list))
    # if clip_val is None:
